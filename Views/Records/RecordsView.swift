            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 6) {
                        ...
                        Image(systemName: vaccinationStatusText == "Fully Vaccinated" ? "checkmark.circle.fill" : "exclamationmark.triangle.fill")
                            .font(.subheadline)
                            .foregroundStyle(vaccinationStatusText == "Fully Vaccinated" ? .green : .orange)
                    }
                }
            ...
            .toolbar {
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        let w = Double(weightText.trimmingCharacters(in: .whitespacesAndNewlines)
...
extension PetModel {
    var guardianContextText: String {
        ...
        if !breed.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { lines.append("- Breed: \(breed)") }
        ...
        if !microchipId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { lines.append("- Microchip ID: \(microchipId)") }
        if !allergies.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { lines.append("- Allergies: \(allergies)") }
        if !notes.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty { lines.append("- Notes: \(notes)") }

        // Recent vaccination
        if let latestVax = vaccinations.sorted(by: { $0.date > $1.date }).first {
            lines.append("")
            lines.append("Latest Vaccination")
            lines.append("- \(latestVax.name) on \(latestVax.date.formatted(date: .abbreviated, time: .omitted))")
        }
        ...
        if !currentMeds.isEmpty {
            ...
            for m in currentMeds {
                ...
                lines.append("- \(m.name) (\(m.dosage), \(m.frequency)) from \(m.startDate.formatted(date: .abbreviated, time: .omitted)) to \(endText)")
            }
        }
        ...
        if let latestWeight = weightEntries.sorted(by: { $0.date > $1.date }).first {
            ...
            lines.append(String(format: "- %.1f kg on %@", latestWeight.weightKg, latestWeight.date.formatted(date: .abbreviated, time: .omitted)))
            if !latestWeight.notes.isEmpty { lines.append("- Notes: \(latestWeight.notes)") }
        }

        lines.append("")
        lines.append("User Question:")
        lines.append("- (Describe symptoms, duration, appetite, energy, vomiting/diarrhea, urination, breathing, and any triggers.)")
