...
struct VaccineBookingView: View {
    ...
    private var dateOptions: [Date] {
        ...
        return (0..<7).compactMap { calendar.date(byAdding: .day, value: $0, to: calendar.startOfDay(for: Date())) }
    }
                ...
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if step == .booking {
                                    ...
                                    ForEach(petOptions, id: \.self) { petName in
                                        Button {
                                            ...
                                            Text("For \(petName)")
                                                ...
                                                .padding(.vertical, 8)
                                                .background(
                                                    selectedPetName == petName ? Color.accentColor : Color.accentColor.opacity(0.12)
                                                )
                                                .clipShape(Capsule())
                                        }
                                        .buttonStyle(.plain)
                        ...
                        Text("Contact Information")
                            .font(.title2.bold())
                        ContactInfoRow(label: "Name", value: owner.name.isEmpty ? "—" : owner.name, icon: "person")
                        ContactInfoRow(label: "Telephone", value: owner.phone.isEmpty ? "—" : owner.phone, icon: "phone")
    ...
    private func confirmAppointment() async {
        ...
        let localTime = MerchantClinicSyncService.composeScheduledAt(date: selectedDate, slot: selectedTimeSlot)
        let chiefComplaint = [
            "OwnerName: \(ownerName)",
            ...
            "Pet: \(petName)",
            additionalNotes.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? nil : "Notes: \(additionalNotes)"
        ]
            .compactMap { $0 }
            .joined(separator: " | ")
        ...
        do {
            ...
            resultAlertMessage = "Appointment sent to testclinics. Merchant clinic panel can now refresh to see this booking."
...
private actor MerchantClinicSyncService {
    ...
    func createTestClinicAppointment(_ payload: MerchantCreateAppointmentRequest) async throws -> String {
    ...
    private func ensureSessionID() async throws -> String {
        if let cachedSession = UserDefaults.standard.string(forKey: sessionStorageKey), !cachedSession.isEmpty {
    ...
    private func loginForTestClinic() async throws -> String {
        ...
        guard (200 ... 299).contains(http.statusCode) else {
            let message = Self.extractErrorMessage(from: data) ?? "Merchant login failed (\(http.statusCode))."
    ...
    private func postAppointment(_ payload: MerchantCreateAppointmentRequest, sessionID: String) async throws -> String {
        var request = URLRequest(url: baseURL.appendingPathComponent("api/merchant/appointments"))
        request.httpMethod = "POST"
        request.setValue("application/json", forHTTPHeaderField: "Content-Type")
        request.setValue(sessionID, forHTTPHeaderField: "X-Session-ID")
        ...
        guard (200 ... 299).contains(http.statusCode) else {
            let message = Self.extractErrorMessage(from: data) ?? "Create appointment failed (\(http.statusCode))."
            throw MerchantClinicSyncError.server(message)
        }
