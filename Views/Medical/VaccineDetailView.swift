...
struct VaccineDetailView: View {
  ...
  private var statusDescription: String {
    if vaccine.isMandatory {
      return
        "This vaccine is commonly required or strongly emphasized by regulation in many regions."
    }

    if vaccine.isCore {
      return
        "Recommended as foundational protection for long-term pet wellness and disease prevention."
    }

    return
      "Best used based on environment, travel patterns, social exposure, and day-to-day lifestyle risk."
  }
  ...
  private var heroCard: some View {
    VStack(alignment: .leading, spacing: 18) {
      ...
      HStack(spacing: 10) {
        DetailMetaPill(
          icon: "sparkles", title: coreLabel, tint: Color(red: 0.98, green: 0.93, blue: 0.80),
          foreground: Color(red: 0.62, green: 0.42, blue: 0.12))
        DetailMetaPill(
          icon: "checkmark.shield", title: mandatoryLabel,
          tint: vaccine.isMandatory
            ? Color(red: 1.00, green: 0.90, blue: 0.90) : Color(red: 0.93, green: 0.96, blue: 0.95),
          foreground: vaccine.isMandatory
            ? Color(red: 0.73, green: 0.22, blue: 0.22) : Color(red: 0.25, green: 0.36, blue: 0.36))
        DetailMetaPill(
          icon: "cross.case", title: "In-Clinic", tint: Color(red: 0.90, green: 0.95, blue: 0.93),
          foreground: Color(red: 0.16, green: 0.43, blue: 0.31))
      }
    }
    .padding(22)
    .background(
      Color.white.opacity(0.84), in: RoundedRectangle(cornerRadius: 30, style: .continuous)
    )
    .overlay(
...
private struct VaccineDetailSectionCard<Content: View>: View {
  ...
  var body: some View {
    ...
    .padding(22)
    .background(
      Color.white.opacity(0.84), in: RoundedRectangle(cornerRadius: 28, style: .continuous)
    )
    .overlay(
  ...
  var body: some View {
    HStack(alignment: .top, spacing: 16) {
      ...
      VStack(spacing: 0) {
        ...
        if !isLast {
          Rectangle()
            .fill(Color(red: 0.88, green: 0.90, blue: 0.91))
            .frame(width: 2)
          // The height of the line will stretch based on the text height
        }
      }

      // Content Column
      VStack(alignment: .leading, spacing: 6) {
        // Add a slight top padding to vertically align the first line of text
        // visually with the center of the 42pt circle icon (half is 21pt)
        Text(title)
          .font(.headline.weight(.semibold))
      ...
      .padding(.bottom, isLast ? 0 : 24)  // Space between rows
    }
    // Set a fixed height on HStack prevents the internal line from expanding to infinity
    .fixedSize(horizontal: false, vertical: true)
  }
}
